...
: "${AKS_CLUSTER_NAME:?}" "${RESOURCE_GROUP_NAME:?}" "${LOCATION:?}" "${UNIQUE_SUFFIX:?}"

MONITOR_NAME="${PROJECT_NAME:0:12}-mon-${UNIQUE_SUFFIX}"
GRAFANA_NAME="${PROJECT_NAME:0:11}-graf-${UNIQUE_SUFFIX}"
...
DASHBOARD_FILE=$(find . -name "dcgm-exporter-dashboard.json" -type f 2>/dev/null | head -n 1)
