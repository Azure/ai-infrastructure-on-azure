...
build_resource_names() {
	...
	if [[ -z "${UNIQUE_SUFFIX:-}" ]]; then
		...
		# Persist suffix to config.sh so all scripts use the same value
		local config_file="${1:-./config.sh}"
		if [[ -f "$config_file" ]]; then
			sed -i "s/^UNIQUE_SUFFIX=\"\"/UNIQUE_SUFFIX=\"${UNIQUE_SUFFIX}\"/" "$config_file"
			echo "  ✓ Saved UNIQUE_SUFFIX=$UNIQUE_SUFFIX to $config_file"
		fi
	fi
