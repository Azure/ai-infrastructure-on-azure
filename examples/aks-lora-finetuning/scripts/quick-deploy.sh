...
# Wait for fine-tuning job to complete (timeout after 90 minutes)
MAX_WAIT=5400
ELAPSED=0
while true; do
	...
	if [ "$status" = "True" ]; then
		...
		echo "❌ Fine-tuning timed out after $((MAX_WAIT / 60)) minutes. Check status: kubectl get job gpt-oss-finetune -n workloads"
		exit 1
	fi
