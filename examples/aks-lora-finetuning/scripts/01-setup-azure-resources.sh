...
# Auto-update config.env with generated suffix if it was auto-generated
if [[ -f "./config.env" ]] && ! grep -q "^UNIQUE_SUFFIX=\"$UNIQUE_SUFFIX\"" ./config.env 2>/dev/null; then
    if grep -q "^UNIQUE_SUFFIX=" ./config.env; then
        grep -v "^UNIQUE_SUFFIX=" ./config.env > ./config.env.tmp
    else
        cp ./config.env ./config.env.tmp
    fi
    echo "UNIQUE_SUFFIX=\"$UNIQUE_SUFFIX\"" >> ./config.env.tmp
    mv ./config.env.tmp ./config.env
    echo "  ✓ Updated config.env with UNIQUE_SUFFIX=\"$UNIQUE_SUFFIX\""
fi
