...
current_nodes=$(kubectl get nodes -l workload=gpu-intensive --no-headers 2>/dev/null | wc -l || echo "0")
echo "   Current GPU nodes: $current_nodes (NC80 = 2 GPUs/node)"

if [ "$current_nodes" -lt 1 ]; then
    echo "   ⚙️  Starting GPU node pool..."
    az aks nodepool update \
        --resource-group "$RESOURCE_GROUP_NAME" \
        --cluster-name "$AKS_CLUSTER_NAME" \
        --name "$AKS_GPU_NODE_POOL_NAME" \
    ...
    echo "   ⏳ Waiting for GPU node (may take 5 min)..."
    timeout=300
    elapsed=0
    while [ "$elapsed" -lt "$timeout" ]; do
        ready=$(kubectl get nodes -l workload=gpu-intensive --no-headers 2>/dev/null | grep -c " Ready " || echo "0")
        [ "$ready" -ge 1 ] && break
        echo -n "."
        sleep 5
        elapsed=$((elapsed + 5))
    ...
    echo ""
    [ "$ready" -ge 1 ] && echo "   ✓ GPU node ready" || echo "   ⚠️  Node still provisioning..."
fi
