...
class ComparisonInference:
    ...
    def generate(
        ...
        # Extract plain tensor if BatchEncoding was returned
        if isinstance(tokenized, torch.Tensor):
            input_ids = tokenized.to(active.device)
        else:
    ...
    def compare(
        ...
        def run(model_name):
            try:
                results[model_name] = self.generate(
                    prompt, reasoning_language, model_name, max_new_tokens
                )
        ...
        # Run in parallel
        threads = [
            threading.Thread(target=run, args=(m,)) for m in ["finetuned", "baseline"]
        ]
        for t in threads:
            t.start()
        for t in threads:
...
if __name__ == "__main__":
    ...
    logger.info(
        "Models ready. Use as module: from model_inference import get_inference"
    )
