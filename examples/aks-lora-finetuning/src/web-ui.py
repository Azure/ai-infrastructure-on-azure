...
def compare():
    try:
        ...
        baseline_prefix = data.get('baseline_prefix', '')  # Additional text for baseline
        ...
        finetuned_result = inference.generate(prompt=prompt, reasoning_language=lang, model="finetuned", max_new_tokens=4096)
        
        # For baseline, prepend custom prefix if provided
        baseline_prompt = f"{baseline_prefix} {prompt}".strip() if baseline_prefix else prompt
        baseline_result = inference.generate(prompt=baseline_prompt, reasoning_language=lang, model="baseline", max_new_tokens=4096)
        
        # Debug: log raw outputs
        logger.info(f"Fine-tuned raw output (first 500 chars): {finetuned_result[:500]}")
            ...
            if model_type == "finetuned":
                ...
                if "assistant<|channel|>commentary" in clean_text:
                    ...
                    if commentary_start != -1:
                        commentary_start = clean_text.find("commentary", commentary_start) + len("commentary")
                        # Find where assistant starts again (final answer)
                        next_assistant = clean_text.find("assistant", commentary_start)
                        if next_assistant != -1:
                            reasoning = clean_text[commentary_start:next_assistant].strip()
                            # Get everything after the second assistant as the answer
                            answer = clean_text[next_assistant:].replace("assistant", "", 1).strip()
                ...
                elif "<|channel|>commentary" in clean_text or "<|channel|>analysis" in clean_text:
                    ...
                    for channel in ["commentary", "analysis"]:
                        ...
                        if channel_tag in clean_text:
                            ...
                            if next_channel != -1:
                                ...
                                for tag in ["<|channel|>final", "<|channel|>commentary", "<|channel|>analysis"]:
        ...
        baseline_display_prompt = f"{baseline_prefix} {prompt}".strip() if baseline_prefix else prompt
