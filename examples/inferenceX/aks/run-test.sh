...
deploy_chart() {
	...
	[[ -f "$base_path" ]] || {
		echo "ERROR: Base values file not found: $base_path"
		exit 1
	}
	[[ -f "$values_path" ]] || {
		echo "ERROR: Recipe values file not found: $values_path"
		exit 1
	}
...
try:
  ...
  print(0)" "$body" 2>/dev/null || echo 0
	...
	[[ -n "$frontend_pod" ]] || {
		echo "ERROR: no Running frontend pod found"
		exit 1
	}
	...
	while ((models_wait < 1800)); do
		...
		if ((models_wait >= 600)) && ! $restarted; then
			...
			if [[ "$endpoints" =~ ^[0-9]+$ ]] && ((endpoints > 0)); then
				...
				frontend_pod=$(resolve_frontend_pod)
				[[ -n "$frontend_pod" ]] || {
					echo "ERROR: frontend pod missing after restart"
					exit 1
				}
