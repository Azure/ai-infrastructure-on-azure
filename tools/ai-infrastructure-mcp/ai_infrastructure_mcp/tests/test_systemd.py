...
"""
    def fake_run(cmd: str):
        assert cmd.startswith('parallel-ssh -i -H "node1 node2" "systemctl is-active sshd"')
...
def test_systemctl_list_units(monkeypatch):
    ...
    import ai_infrastructure_mcp.tools.command_wrapper as cw
    monkeypatch.setattr(cw, 'run_login_command', fake_run)
    result = systemd.systemctl(['h1'], ['list-units', '--type=service', '--state=active'])
    
    assert result['success'] is True
    assert result['raw_output'] == "UNIT                     LOAD   ACTIVE SUB     DESCRIPTION"
...
def test_systemctl_show_properties(monkeypatch):
    ...
    import ai_infrastructure_mcp.tools.command_wrapper as cw
    monkeypatch.setattr(cw, 'run_login_command', fake_run)
    result = systemd.systemctl(['h1'], ['show', 'mysql', '--property=ActiveState,SubState'])
...
"""
    def fake_run(cmd: str):
        assert cmd.startswith('parallel-ssh -i -H "nodeA nodeB" "journalctl -u sshd -n 1"')
        return sample_output
    monkeypatch.setattr(cw, 'run_login_command', fake_run)
...
def test_systemctl_complex_arguments(monkeypatch):
    ...
    monkeypatch.setattr(cw, 'run_login_command', fake_run)
    result = systemd.systemctl(['h1'], [
        'list-units', '--type=service', '--state=failed', '--no-pager'
    ])
...
def test_journalctl_complex_arguments(monkeypatch):
    ...
    def fake_run(cmd: str):
        assert "journalctl -u apache2 --since '2024-01-01 00:00:00' --until '2024-01-31 23:59:59' --no-pager" in cmd
        return "mock complex log output"
    import ai_infrastructure_mcp.tools.command_wrapper as cw
    monkeypatch.setattr(cw, 'run_login_command', fake_run)
    result = systemd.journalctl(['h1'], [
        '-u', 'apache2', '--since', '2024-01-01 00:00:00', '--until', '2024-01-31 23:59:59', '--no-pager'
    ])
...
def test_systemctl_arguments_with_spaces(monkeypatch):
    ...
    import ai_infrastructure_mcp.tools.command_wrapper as cw
    monkeypatch.setattr(cw, 'run_login_command', fake_run)
    result = systemd.systemctl(['h1'], ['show', 'my service with spaces', '--property=Description'])
...
def test_journalctl_arguments_with_spaces(monkeypatch):
    ...
    def fake_run(cmd: str):
        assert "journalctl --since '2024-01-01 12:00:00' --until '2024-01-01 13:00:00'" in cmd
        return "-- Logs begin at..."
    import ai_infrastructure_mcp.tools.command_wrapper as cw
    monkeypatch.setattr(cw, 'run_login_command', fake_run)
    result = systemd.journalctl(['h1'], ['--since', '2024-01-01 12:00:00', '--until', '2024-01-01 13:00:00'])
