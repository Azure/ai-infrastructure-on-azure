...
def read_file_content(
    ...
    if action == "count":
        ...
        cmd = f"wc -l < {escaped_path}" if mode == "lines" else f"wc -c < {escaped_path}"
        ...
        cmd_parts = ["grep", "-n", "-E"]
        if lines_before > 0: cmd_parts.extend(["-B", str(lines_before)])
        if lines_after > 0: cmd_parts.extend(["-A", str(lines_after)])
